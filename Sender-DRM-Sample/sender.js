...
var mediaURLs = [
    ...
    'http://brightcove04.brightcove.com/21/76009399001/201504/1323/76009399001_4187465367001_811357017692.mp4',
    ...
    'http://brightcove01.brightcove.com/16/4276901753001/201506/1924/4276901753001_4316973319001_4316929298001.mpd'];
...
var mediaLicenseServerURLs = [
    'https://wvlic.brightcove.com/widevine/cypherpc/cgi-bin/GetEMMs.cgi',
    '',
    '',
    '',
    '',
    ...
    '',
    '',
    'https://wvlic.brightcove.com/proxy/4316929298001'];

    /*http://widevine-proxy.appspot.com/proxy*/
    /*https://wvlic.brightcove.com/proxy/4304516800001*/
    /*https://wvlic.brightcove.com/proxy/4316929298001*/
...
function setColor(color) {
  if (session != null) {
    console.log(color);
    sendMessage({'type': 'color', 'value': color});
  } else {
    alert('First connect to a Cast device.');
  }
}
...
function showControls() {
  if (session != null) {
    sendMessage({'type': 'control', 'value': 'control'});
  } else {
    alert('First connect to a Cast device.');
  }
}

function setMode(numberStr) {
  if (session != null) {
    sendMessage({'type': 'mode', 'value': numberStr});
  } else {
    alert('First connect to a Cast device.');
  }
}
...
function back30(){
  console.log(pct);
  var pctChange = (3000)/(parseInt($('#duration').html()));
  var current = parseInt($('#progress').val());
  var pct = current - pctChange;
  if(pct>0){
    seekMedia(pct);
  }
...
function setMaxBandwidth() {
  ...
  if (maxBW.length > 0 && !isNaN(maxBW)) {
    sendMessage({'type': 'bandwith', 'value': maxBW});
  }
